...
def main(argv=None):
    """
    Main function to generate H5 file from ECG records.
    Can be called with a list of arguments.
    """
    if argv is None:
        argv = sys.argv[1:]

    parser = argparse.ArgumentParser(description='Read XML muse and plot the first lead of the obtained ECG.')
    parser.add_argument('input_file', type=str, help='path to RECORDS file.')
    parser.add_argument('out_file', type=str, help='output file containing the plots.')
    parser.add_argument('--root_dir', type=str, help='The root directory which the relative paths refer to.')
